...
class NoteService:
    ...
    def get_notes(self, skip: int = 0, limit: int = 100) -> list[Note] | None:
        self._db.query(models.Note).offset(skip).limit(limit).all()

    def create_note(self, note_id: int, note: schemas.NoteUpdate) -> Note | None:
